...
public class Popp extends AnchorPane {
	 ...
	 * 
	 * @param popupContainer the container where the popup will be added (e.g the
	 *                       root of the scene)
	 * @param content        the node that will be shown in the popup
	 */
	public Popp(Pane popupContainer, Region content) {
		initialize();
		setContent(content);
	 ...
	 * @param popupContainer parent the show the popup inside (e.g the root of the
	 *                       scene)
	 */
	...
	public void show(PopupVPosition vAlign, PopupHPosition hAlign, double initOffsetX, double initOffsetY) {
		...
		if (!popupContainer.getChildren().contains(this))
		...
		if (hAlign.equals(PopupHPosition.RIGHT)) {
			...
			contentHolder.translateXProperty()
					.bind(Bindings.createDoubleBinding(
							() -> -content.getWidth() + source.getBoundsInLocal().getWidth() + offsetX,
							content.widthProperty(), source.boundsInLocalProperty()));
		} else {
		...
		if (vAlign.equals(PopupVPosition.BOTTOM)) {
			...
			contentHolder.translateYProperty()
					.bind(Bindings.createDoubleBinding(
							() -> -content.getHeight() + source.getBoundsInLocal().getHeight() + offsetY,
							content.heightProperty(), source.boundsInLocalProperty()));
		} else {
	...
	/**
	 * close the popup, by default the popup will close when clicking outside the
	 * popup content and inside its container
	 */
	public void close() {
		animation.setRate(-1);
		animation.play();
			...
			super(Popp.this,
					new Timeline(
							new KeyFrame(Duration.ZERO,
									new KeyValue(Popp.this.visibleProperty(), false, Interpolator.EASE_BOTH),
									new KeyValue(content.opacityProperty(), 0, Interpolator.EASE_BOTH)),
							new KeyFrame(Duration.millis(10),
									new KeyValue(Popp.this.visibleProperty(), true, Interpolator.EASE_BOTH),
									new KeyValue(Popp.this.opacityProperty(), 0, Interpolator.EASE_BOTH),
							...
							new KeyFrame(Duration.millis(1000),
									...
									new KeyValue(Popp.this.opacityProperty(), 1, Interpolator.EASE_BOTH),
